                ...
                if selected_id:
                    ...
                    if customer:
                        with st.form("update_customer_form"):
                            ...
                            if submitted:
                                try:
                                    Customers.update_customer(selected_id, name, contact)
                            ...
                            if selected_product:
                                ...
                                if product_details:
                                    ...
                                    with st.form("add_item_form"):
                                        ...
                                        if submitted_item:
                                            try:
                                                available_quantity = int(product_details[4])

                                                # Check stock before making the sale 
                                                if quantity > available_quantity:
                                                    st.error(f"❌Insufficient stock. Only {available_quantity} units of '{product_details[1]}' are available.")

                                                elif available_quantity == 0:
                                                    st.error(
                                                        f"❌ '{product_details[1]}' is OUT OF STOCK.")
                                                    ...
                                                    if new_quantity == 0:
                                                        st.warning(
                                                            f"⚠️ '{product_details[1]}' is now OUT OF STOCK."
                                                        )
                                                    elif new_quantity < 10:
                                                        st.warning(
                                                            f"⚠️ Low stock: only {new_quantity} "
                                                            f"units of '{product_details[1]}' remaining."
                                                        )
                                                    else:
                                                        st.success(
                                                            f"Item added successfully! "
                                                            f"{new_quantity} units remaining."
                                                        )
                                            except Exception as e:
        ...
        try:
            ...
            if low_stock:
                for item in low_stock:
                    if item[1] <= 0:
                        st.error(
                            f"🔴 {item[0]}: OUT OF STOCK"
                        )
                    else:
                        st.warning(
                            f"⚠️ {item[0]}: "
                            f"{item[1]} units remaining"
                        )
            else:
                st.success(
                    "No low stock items. "
